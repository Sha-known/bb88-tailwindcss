      ...
      <div class="relative z-10 max-w-275 mx-auto">
        ...
        <div class="flex justify-center mb-10 w-full px-2 font-sans">
          <div class="filter-scroll flex items-center bg-white text-[#458B7B] rounded-full p-2 shadow-[2px_8px_15px_-3px_rgba(0,0,0,0.25)] gap-1 md:gap-4.5 overflow-x-auto w-full md:w-auto">
            ${filters.map(f => `
              <button class="filter-btn shrink-0 ${f.padding} py-1 rounded-lg text-[15px] transition-all duration-200 ${f.isActive ? 'bg-green-dark text-white' : 'text-green-dark hover:bg-gray-50'}" data-filter="${f.id}">
                ${f.label}
              </button>
            `).join('')}
          </div>
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 mb-10">
          ${projects.map(proj => `
            <div class="portfolio-card ${proj.isHidden ? 'extra-card hidden' : ''} group relative rounded-[20px] cursor-pointer shadow-[0_6px_22px_rgba(0,0,0,0.12)]" data-category="${proj.category}" style="border: 5px solid #48887B;">
        ...
        <div class="col-span-full flex justify-center mt-10">
          <button id="see-more-btn" class="inline-flex items-center justify-center px-10 py-3 bg-white text-green-dark font-semibold text-[15px] border-2 border-green-dark rounded-full transition-all duration-300 hover:bg-green-dark hover:text-white hover:shadow-lg">See More</button>
        </div>
      </div>
    `;
